...
const subexams = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`http://localhost:8000/GetSubExams/${id}`)
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold lg:ml-160 ml-22 text-black lg:mb-8 mb-4 lg:mt-30 mt-40">
        Topics
      </h1>
      ...
      <div className="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 lg:ml-20 ml-7 justify-center lg:gap-6 gap-3 lg:mt-10 mt-4">
        {cards.map((card, index) => (
          <Allcards key={index} name={card.name} route={`/minorexams/${card.id}`} />
        ))}
      </div>
    </div>
  );
